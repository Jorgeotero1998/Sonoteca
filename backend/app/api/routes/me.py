...
async def list_favorites(
    ...
    for r in rows:
        ...
        out.append(
            HydratedOut(
                ref=f"{r.provider}:{r.provider_id}", type=r.type, created_at=r.created_at, item=item
            )
        )
    return out
...
async def remove_favorite(
    ...
    await db.execute(
        delete(UserFavorite).where(
            ...
            UserFavorite.provider == provider,
            UserFavorite.provider_id == pid,
        )
    )
    await db.commit()
    return {"ok": True}
    ...
    rows = (
        (
            await db.execute(
                ...
                .order_by(desc(UserLibrary.created_at))
            )
        )
        .scalars()
        .all()
...
async def remove_library(
    ...
    await db.execute(
        delete(UserLibrary).where(
            ...
            UserLibrary.provider == provider,
            UserLibrary.provider_id == pid,
        )
    )
    await db.commit()
    return {"ok": True}
...
async def add_history(
    payload: RefIn, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)
):
